...
export const logInUser = async (logInData) => {
  ...
  const res = await handleAPICall(`${urls.users}/login`, "POST", body);
  if (res.status == 200) {
    localStorage.setItem("accessToken", res?.data?.accessToken);
    localStorage.setItem("refreshToken", res?.data?.refreshToken);
    localStorage.setItem("user", res && res.data && JSON.stringify(res?.data?.user));
  }
...
export const registerUser = async (userData) => {
  const body = userData;
  const res = await handleAPICall(`${urls.users}/register`, "POST", body);
  return res;
};

export const logOutUser = async () => {
  const res = await protectedAPICall(`${urls.users}/logout`, "POST");
  if (res.status == 200) {
